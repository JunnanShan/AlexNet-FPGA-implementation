...
#pragma HLS INTERFACE s_axilite port=return bundle=control
	...
	DataType inp_image_local[INP_IMAGE_SIZE * INP_IMAGE_SIZE * INP_IMAGE_CHANNEL];
...
//	DataType window[INP_IMAGE_CHANNEL][FILTER_SIZE][FILTER_SIZE];
...
//	DataType conv_out[INP_IMAGE_CHANNEL][FILTER_SIZE][FILTER_SIZE];
//	#pragma HLS ARRAY_PARTITION variable=conv_out complete dim=0
//	DataType conv_out[FILTER_BATCH][OUT_IMAGE_SIZE][OUT_IMAGE_SIZE];
//	DataType conv_out[FILTER_BATCH*OUT_IMAGE_SIZE*OUT_IMAGE_SIZE];
...
L1_1:	  for (int c = 0; c < INP_IMAGE_SIZE * INP_IMAGE_SIZE * INP_IMAGE_CHANNEL; c++)
	    {
#pragma HLS PIPELINE
...
L4: for (int batch = 0; batch < FILTER_BATCH; batch++){

		F1_1:for (int h = 0; h <  FILTER_SIZE * FILTER_SIZE * INP_IMAGE_CHANNEL; h++){
#pragma HLS PIPELINE
					filter_local[h] = filter[batch*INP_IMAGE_CHANNEL*FILTER_SIZE*FILTER_SIZE + h];
...
#pragma HLS PIPELINE
		...
		C1_1:for (int chan = 0; chan < INP_IMAGE_CHANNEL; chan++){
#pragma HLS PIPELINE
...
//					DataType m = conv_out[((row-FILTER_SIZE+1)/STRIDE)*OUT_IMAGE_SIZE + (col-FILTER_SIZE+1)/STRIDE];
